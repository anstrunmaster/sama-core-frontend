...
import {
  ...
  Mail, FileText, Lock, ThumbsUp, ThumbsDown, Trash2, RefreshCw,
} from 'lucide-react'
...
export default function CotizacionDetailPage() {
    ...
    <DashboardLayout>
      <div className="p-6">
        <div className="card py-16 text-center">
          ...
          <p className="text-sm text-ink-tertiary">Cargando cotización...</p>
        </div>
      </div>
    </DashboardLayout>
  )
  ...
  const canEdit    = ['DRAFT', 'SENT'].includes(data.status)
      ...
      <div className="p-6 space-y-5">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            {data.status === 'DRAFT' && (
              <button
                onClick={() => doAction('sent', () => quotationsApi.changeStatus(id, 'SENT'), 'Cotización marcada como enviada')}
                disabled={!!actionBusy}
            ...
            {data.status === 'SENT' && (
              <>
                <button
                  onClick={() => doAction('approved', () => quotationsApi.changeStatus(id, 'APPROVED'), 'Cotización aprobada')}
                  disabled={!!actionBusy}
                ...
                <button
                  onClick={() => doAction('rejected', () => quotationsApi.changeStatus(id, 'REJECTED'), 'Cotización rechazada')}
                  disabled={!!actionBusy}
            ...
            {data.status === 'INVOICED' && data.invoice_access_key && (
              <Link
                href={`/facturas?search=${data.invoice_access_key}`}
        ...
        <div className="card p-5">
          <h2 className="text-[10px] font-semibold uppercase tracking-widest text-ink-tertiary mb-3">
            Comprador
          </h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <InfoField label="Nombre" value={data.buyer_name} />
            <InfoField label="ID" value={data.buyer_id} />
            <InfoField label="Email" value={data.buyer_email} />
            <InfoField label="Forma de pago" value={data.payment_method} />
          </div>
          {data.notes && (
            ...
            <table className="w-full">
              <thead className="bg-surface-raised">
                <tr className="border-b border-edge-subtle">
                  {['Código', 'Descripción', 'Cant.', 'P. Unit.', 'Desc.', 'IVA', 'Subtotal'].map((h) => (
              ...
              <tbody>
                {items.map((item: any, i: number) => {
                  const ivaPct  = item.impuestos?.[0]?.codigoPorcentaje === '4' ? 15 : 0
                  const base    = item.cantidad * item.precioUnitario - item.descuento
                  const ivaVal  = ivaPct === 15 ? base * 0.15 : 0
                  ...
                  return (
                    <tr key={i} className="border-b border-edge-subtle last:border-0">
                      ...
                      <td className="px-4 py-3 text-sm text-ink-primary">{item.descripcion}</td>
                      ...
                      <td className="px-4 py-3 text-sm text-ink-tertiary">{ivaPct}%</td>
              ...
              <tfoot className="bg-edge-subtle">
                <tr>
                  <td colSpan={6} className="px-4 py-2.5 text-right text-[10px] font-semibold uppercase tracking-widest text-ink-tertiary">Subtotal</td>
            ...
            <div className="flex items-start gap-4 mb-5">
              ...
              <div>
                ...
                <p className="text-sm text-ink-tertiary mt-1">
                  Esta acción emite la factura al SRI y consume el secuencial. No se puede deshacer.
                </p>
              </div>
            </div>
...
function InfoField({ label, value }: { label: string; value: string | null | undefined }) {
  return (
    <div>
      ...
      <p className="text-sm text-ink-primary">{value || <span className="text-ink-ghost">—</span>}</p>
    </div>
  )
}
