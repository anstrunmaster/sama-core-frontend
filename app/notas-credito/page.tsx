...
import {
  ...
  FileCode2, AlertTriangle, CheckCircle2, Search, RefreshCw, Plus
} from 'lucide-react'
...
function getUser(): any {
  try {
    ...
    if (saasAuth) { const p = JSON.parse(saasAuth); return p.state?.user ?? p.user ?? p }
...
function getNoteNumber(note: CreditNote) {
  return `${note.establishment}-${note.emission_point}-${note.sequential}`
}
function getCustomerName(note: CreditNote) {
  return note.invoice_data?.razonSocialComprador || note.invoice_data?.identificacionComprador || '—'
}
function getTotal(note: CreditNote) {
  return note.invoice_data?.valorModificacion ?? note.invoice_data?.importeTotal ?? 0
}
...
export default function NotasCreditoPage() {
  ...
  const fetchNotes = useCallback(async (page = 1) => {
    ...
    try {
      ...
      if (payload.success !== false) {
        ...
        setError(payload.error || 'Error al cargar notas de crédito')
      }
  ...
  const downloadXml = async (note: CreditNote) => {
    ...
    try {
      ...
      a.href = url; a.download = `nota-credito-${getNoteNumber(note)}.xml`; a.click()
  ...
  const filtered = notes.filter(n => {
    ...
    return getNoteNumber(n).includes(q) || getCustomerName(n).toLowerCase().includes(q) || (n.reference_access_key || '').includes(q)
  })
      ...
      <div className="p-6 space-y-5">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-bold text-ink-primary">Notas de Crédito</h1>
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => router.push('/notas-credito/nueva')}
        ...
        <div className="relative">
          ...
          <input type="text" placeholder="Buscar por número, cliente o clave de factura original..."
            ...
            <table className="w-full">
              <thead className="bg-surface-raised">
                <tr className="border-b border-edge-subtle">
                  {['N° Nota', 'Cliente', 'Factura Original', 'Motivo', 'Total', 'Fecha', 'Estado', 'Acciones'].map(h => (
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={8} className="px-4 py-16 text-center">
                      ...
                      <p className="text-sm text-ink-tertiary">
                        {search ? 'No se encontraron notas de crédito' : 'Aún no hay notas de crédito emitidas'}
                      </p>
                      <button onClick={() => router.push('/notas-credito/nueva')}
                  ...
                  <tr key={note.id} className="border-b border-edge-subtle hover:bg-edge-subtle transition-colors">
                    ...
                    <td className="px-4 py-3.5">
                      <div className="font-mono text-xs text-ink-secondary truncate max-w-[140px]" title={note.reference_access_key || ''}>
                        {note.reference_access_key ? note.reference_access_key.slice(0, 20) + '...' : '—'}
                      </div>
                    </td>
                    <td className="px-4 py-3.5">
