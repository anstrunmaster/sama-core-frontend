...
import {
  ...
  TrendingUp, Clock, AlertTriangle, CheckCircle2, FileText,
} from 'lucide-react'
import { DashboardLayout } from '@/components/layout/DashboardLayout'
import {
  cxcApi,
  fmtMoney, fmtDate, bucketColor,
  type ReceivablesResponse, type ReceivableRow,
} from './api-cxc'
...
export default function CxCPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError('')
    try {
      const res = await cxcApi.getReceivables({
  ...
  const rows: ReceivableRow[] = (data?.data ?? []).filter(r => {
    if (!search) return true
    const q = search.toLowerCase()
    return (
      r.customer?.name.toLowerCase().includes(q) ||
      r.customer?.identification.includes(q) ||
        ...
        <div className="flex items-center justify-between">
          <div>
            <div className="flex items-center gap-2 mb-0.5">
              ...
              <h1 className="text-lg font-bold text-ink-primary">Cuentas por Cobrar</h1>
            </div>
            <p className="text-sm text-ink-tertiary mt-0.5">
              Cartera pendiente de cobro · {summary?.count ?? 0} facturas
            </p>
          </div>
        ...
        {summary && (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
            <div className="lg:col-span-2 card p-4">
              <p className="text-[10px] font-semibold uppercase tracking-widest text-ink-tertiary mb-1">Total por cobrar</p>
              <p className="text-2xl font-bold text-ink-primary tabular-nums">{fmtMoney(summary.total_receivable)}</p>
              <p className="text-[11px] text-ink-ghost mt-1">{summary.count} facturas pendientes</p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
            <div className="relative md:col-span-2">
              ...
              <input type="text" value={search} onChange={e => setSearch(e.target.value)}
                placeholder="Buscar cliente, RUC o N° factura..." className="field pl-10 w-full" />
              {search && (
        ...
        <div className="card overflow-hidden">
            ...
            <table className="w-full">
              <thead className="bg-surface-raised">
                <tr className="border-b border-edge-subtle">
                  {['Factura','Cliente','Fecha','Total','Cobrado','Retenido','Saldo','Antigüedad',''].map(h => (
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={9} className="px-4 py-16 text-center">
                      ...
                      <p className="text-sm font-medium text-ink-primary mb-1">
                        {bucket || search ? 'Sin resultados para los filtros aplicados' : '¡Sin cartera pendiente!'}
                      </p>
                    </td>
                  </tr>
                ) : rows.map(r => {
                  ...
                  return (
                    <tr key={r.invoice_id} className="border-b border-edge-subtle hover:bg-edge-subtle transition-colors">
                      <td className="px-4 py-3.5">
                        <div className="flex items-center gap-1.5">
                          <FileText className="w-3.5 h-3.5 text-ink-ghost shrink-0" />
                          <span className="font-mono text-xs font-semibold text-ink-primary">{r.sequential}</span>
                        </div>
                      </td>
                      <td className="px-4 py-3.5">
                        <p className="text-sm font-medium text-ink-primary truncate max-w-[180px]">{r.customer?.name ?? '—'}</p>
                        <p className="text-[11px] text-ink-ghost font-mono">{r.customer?.identification ?? ''}</p>
                      </td>
                      ...
                      <td className="px-4 py-3.5 text-sm font-mono text-amber-600 dark:text-amber-400 whitespace-nowrap">{r.withheld > 0 ? fmtMoney(r.withheld) : '—'}</td>
                      ...
                      <td className="px-4 py-3.5">
                        {r.pdf_s3_url && (
                          <a href={r.pdf_s3_url} target="_blank" rel="noopener noreferrer"
                            className="text-[11px] text-blue-600 dark:text-blue-400 hover:underline">PDF</a>
                        )}
                      </td>
                    </tr>
                  )
          ...
          {data && data.pagination.pages > 1 && (
            <div className="flex items-center justify-between px-4 py-3 border-t border-edge-subtle">
              <span className="text-xs text-ink-tertiary">Página {data.pagination.page} de {data.pagination.pages} · {data.pagination.total} facturas</span>
